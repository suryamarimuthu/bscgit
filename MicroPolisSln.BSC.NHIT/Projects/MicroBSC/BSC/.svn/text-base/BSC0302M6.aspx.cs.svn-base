...
public partial class BSC_BSC0302M6 : AppPageBase
{
    #region ==========================[변수선언]================
    public string ICCB1
    {
    ...
    private void drawPoint()
    {
        ...
        MSCharts.DundasChartBase(chartScore, ChartImageType.Jpeg
            , DataTypeUtility.GetToInt32(this.chartScore.Width.Value)
            , DataTypeUtility.GetToInt32(this.chartScore.Height.Value)
    ...
    protected void ugrdPlan_InitializeLayout(object sender, LayoutEventArgs e)
    {
        ...
        ugrdPlan.DisplayLayout.Bands[0].HeaderLayout.Reset();

        int iIndex = 0;
        Infragistics.WebUI.UltraWebGrid.ColumnHeader ch;

        foreach (Infragistics.WebUI.UltraWebGrid.UltraGridColumn c in ugrdPlan.DisplayLayout.Bands[0].Columns)
        {
            c.Header.RowLayoutColumnInfo.OriginY = 1;
            c.Header.RowLayoutColumnInfo.OriginX = iIndex;
            iIndex++;
        ...
        //ch.RowLayoutColumnInfo.SpanX = 1;
        //ch.Style.Height = Unit.Pixel(20);
        //ugrdPlan.DisplayLayout.Bands[0].HeaderLayout.Add(ch);
        //ugrdPlan.DisplayLayout.Bands[0].HeaderStyle.HorizontalAlign = HorizontalAlign.Center;
        ...
        ch = ugrdPlan.DisplayLayout.Bands[0].Columns[0].Header;
        ch.RowLayoutColumnInfo.OriginY = 0;
        ch.RowLayoutColumnInfo.OriginX = 0;
        ch.RowLayoutColumnInfo.SpanY = 2;

        ch = ugrdPlan.DisplayLayout.Bands[0].Columns[7].Header;
        ch.RowLayoutColumnInfo.OriginY = 0;
        ch.RowLayoutColumnInfo.OriginX = 7;
        ch.RowLayoutColumnInfo.SpanY = 2;

        ch = ugrdPlan.DisplayLayout.Bands[0].Columns[8].Header;
        ch.RowLayoutColumnInfo.OriginY = 0;
        ch.RowLayoutColumnInfo.OriginX = 8;
        ch.RowLayoutColumnInfo.SpanY = 2;

        for (int i = 0; i < ugrdPlan.DisplayLayout.Bands[0].Columns.Count; i++)
        {
            if (i == 0)
            {
                ugrdPlan.DisplayLayout.Bands[0].Columns[i].Width = 80;
                ugrdPlan.DisplayLayout.Bands[0].Columns[i].CellStyle.HorizontalAlign = HorizontalAlign.Center;
            }
            else if (i > 0 && i < 5)
            {
                ugrdPlan.DisplayLayout.Bands[0].Columns[i].DataType = "System.Double";
                ugrdPlan.DisplayLayout.Bands[0].Columns[i].Format = "###,###,###,###,###,###,##0.####";
                ugrdPlan.DisplayLayout.Bands[0].Columns[i].CellStyle.HorizontalAlign = HorizontalAlign.Right;
            }
            else
            {
                //ugrdPlan.DisplayLayout.Bands[0].Columns[i].Width = 100;
                //ugrdPlan.DisplayLayout.Bands[0].Columns[i].CellStyle.HorizontalAlign = HorizontalAlign.Left;
            }
        }
    }
    ...
    protected void ugrdPlan_InitializeRow(object sender, RowEventArgs e)
    {
        ...
        {
            e.Row.Cells.FromKey("MS_TARGET").Value = "-";
            e.Row.Cells.FromKey("TS_TARGET").Value = "-";
        }
    }
    ...
    private void DoBinding_KpiTarget()
    {
        ...
        DataTable dt_target = objBSC.GetAllList(this.IEstTermRefID, this.IKpiRefID, 1).Tables[0];
