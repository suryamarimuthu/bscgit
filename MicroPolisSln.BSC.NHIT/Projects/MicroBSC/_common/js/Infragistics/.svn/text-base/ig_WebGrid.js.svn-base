...
function igtbl_onBodyResize()
{
		...
		for (var gridId in igtbl_gridState)
	{
		...
		if (igtbl_inEditMode(gridId))
		{
			...
			if (edit && edit.Object && edit.Object.setDropDownVisible)
				break;
...
function igtbl_lineupHeaders(colId, band) {
	...
	if (band.Index == 0 && !band.IsGrouped && gs.StationaryMargins > 0)
	{
		...
		if (ig_shared.IsIEStandards)
			cells[0] = te.childNodes[1].firstChild;
		if (gs.StatHeader)
		{
			cg[1] = gs.StatHeader.Element.previousSibling;
			
			if (ig_shared.IsIEStandards)
				cells[1] = gs.StatHeader.Element.firstChild;
		}
		if (gs.StatFooter)
		{
				...
				|| (gs.Rows.FilterRow && gs.Rows.FilterRow.isFixedBottom()))
			{
				...
				if (ig_shared.IsIEStandards)
					cells[cells.length] = gs.StatFooter.Element.previousSibling.firstChild;
			}
			else
			{
				...
				if (ig_shared.IsIEStandards)
					cells[cells.length] = gs.StatFooter.Element.firstChild;
			}
		}
		stat = true;
	}
		...
		for (var j = 0; j < cg.length; j++)
		{
			...
			if (hasPercW)
			{
				for (var i = 0; i < cg[j].childNodes.length; i++)
				{
					...
					if (ig_shared.IsIEStandards)
						cg[j].childNodes[i].oldWidth = cells[j].childNodes[i].offsetWidth;
				}
			}
...
{	
	...
	var childLeft = igtbl_getLeftPos(child) + childLeftAdj;
	...
	var parentRight = scrParent ? (igtbl_getLeftPos(scrParent) + scrParent.clientWidth + (!ig_csom.IsIE7Compat ? scrParent.scrollLeft : 0)) : (parentLeft + parent.clientWidth);
