...
function()
{
	...
	var adj = 0;
	if ((this.Row.IsFilterRow || this.Row.IsAddNewRow) && this.Row.isFixed())
	{
...
function(width)
{
	...
	if (c1w > 0 && !igtbl_fireEvent(gn, gs.Events.BeforeColumnSizeChange, "(\"" + gn + "\",\"" + colObj.id + "\"," + c1w + ")"))
	{
		...
		if (colEl && gs.UseFixedHeaders && isIECompat)
		{
			...
			if (colFoots)
			{
				...
				for (var i = 0; i < colFoots.length; i++)
				{
					...
					if (cg && cg.tagName == "COLGROUP")
					{
						...
						if ((ig_csom.IsFireFox || ig_csom.IsIEStandards) && fixedColIndex != null && fixedColIndex > -1 && fixedColIndex < cg.childNodes.length)
					...
					if (nfth && this.Band.Index == 0 && this.Band.Index == 0 && gs.StatFooter)
					{
						...
						if (cg)
						{
							...
							var c = cg.childNodes[nfthCellIndex + colFoots[i].cellIndex];
							if (c)
							{
		...
		for (var i = 0; i < columns.length; i++)
		{
			...
			if (columns[i].style.display == "none" || ig_csom.IsFireFox || ig_csom.IsIEStandards)
			{
				var itr = 0;
				...
				if (fixed)
				{
					...
					if (nfth)
					{
						...
						if (cg)
						{
							...
							var c = cg.childNodes[nfthCellIndex + colIndex];
							
							c.style.width = c1w + corr + "px";
							c.width = c1w + corr + "px";  
						}
...
function(elem)
{
	...
	if (divElem.clientHeight < divElem.scrollHeight)
	{
		...
		if (elem.Element.parentNode.style.width == "100%")
		{
			...
			if (igtbl_isXHTML && ig_shared.IsIE7Compat)
				elem.Element.parentNode.style.paddingRight = (scrollbarWidth > 0) ? scrollbarWidth + "px" : "";
			else
			{
	...
	else
	{
		if (elem.Element.parentNode.style.width == "100%")
		{
			...
			if (igtbl_isXHTML && ig_shared.IsIE7Compat)
				elem.Element.parentNode.style.paddingRight = "";
			else
		...
		if (obj && (ch = obj._Changes["FilterColumn"]))
		{
			if (ch.length)
			{
				for (var i = ch.length - 1; i >= 1; i--)
				{
					...
					var values = (ch[i].Node.Value) ? ch[i].Node.Value.split("%01") : ch[i].Node.props;
			...
			else
			{
				...
				var values = (ch.Node.Value) ? ch.Node.Value.split("%01") : ch.Node.props;
				if (values[2] == parentRowIds[0] && values[3] == parentRowIds[1])
					ch.remove();
			}
		}
...
function(scrollLeft, force)
{
	...
	)
	{
		...
		this._alignFilterRow(this.Rows);
		return;
	}
...
{
    /* 2011-08-24 수정시작 : defineGetter가 정의되어 있는 경우만 실행하도록 수정 */
    if (igtbl_XSLTProcessor.prototype.__defineGetter__) {
