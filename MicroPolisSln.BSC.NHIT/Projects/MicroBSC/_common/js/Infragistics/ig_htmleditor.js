...
function iged_new(id,ta,tb,p1,p2,p3,p4,p5)
{
	...
	this.focus=function(){if(this._foc<2)try{if(this._ie)this._elem.focus();else this._win().focus();}catch(i){}}
	...
	this._pos=function(evt,pan,flag)
	{
		...
		s=pan.style;
		f=this._elem0;if(!f)f=this._elem;
		z=this._maxZ(pan,s.zIndex);f=this._maxZ(f);
		s.position="absolute";s.display="";s.visibility="visible";s.zIndex=(z>f)?z:f;
		panW=pan.offsetWidth;panH=pan.offsetHeight;
		ieBug=this._ie;
		if(flag==3){x0=evt.clientX+body.scrollLeft+10;y0=evt.clientY+body.scrollTop;elemH=10;if(!ieBug)y0+=panH;}
		var ok=0,moz=0;pe=e;
		while(e)
		{
...
function iged_mainEvt(e)
{
	...
	/*this._foc=-1:never blur, 0:blur, 1:focus before blur, 2:focus after blur*/
	if(t==8&&!inElem&&(o._foc<1)&&!(src.unselectable&&iged_all._cur==o))window.setTimeout("iged_all._cur.focus()",0);
	iged_all._cur=o;
}
