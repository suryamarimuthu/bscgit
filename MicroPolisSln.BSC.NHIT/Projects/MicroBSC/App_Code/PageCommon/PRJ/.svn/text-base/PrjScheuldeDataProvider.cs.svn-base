...
namespace MicroBSC.Common
{
    ...
    /// </summary>
    public class PrjScheuldeDataProvider : WebScheduleDataProviderBase, IDataFetch, IDataUpdate
    {
        private DataTable _dataSchedule = null;

        public PrjScheuldeDataProvider()
        {
        }

        public PrjScheuldeDataProvider(DataTable dt)
        {
            _dataSchedule = dt;
        }
            ...
            foreach (DataRow row in _dataSchedule.Rows)
            {
                if (row["PLAN_START_DATE"] != DBNull.Value && row["PLAN_END_DATE"] != DBNull.Value)
                {
                    if (row["UP_TASK_REF_ID"].ToString() != "0")
                    {
                        ...
                        appointment.Subject = row["TASK_NAME"].ToString();
            ...
            foreach (DataRow row in _dataSchedule.Rows)
            {
                if (row["PLAN_START_DATE"] != DBNull.Value && row["PLAN_END_DATE"] != DBNull.Value)
                {
                    if (row["UP_TASK_REF_ID"].ToString() != "0")
                    {
                        Appointment appointment = new Appointment(this.WebScheduleInfo);
                        appointment.Key = row["TASK_REF_ID"].ToString() + "|" + row["PRJ_REF_ID"].ToString();
                        appointment.Subject = row["TASK_NAME"].ToString() + "종료";
                        appointment.StartDateTime = StringToDate(row["PLAN_END_DATE"].ToString());


                        appointment.EndDateTime = StringToDate(row["PLAN_END_DATE"].ToString());
                        //appointment.EndDateTime = StringToDate(row["PLAN_END_DATE"].ToString()).AddDays(1);
